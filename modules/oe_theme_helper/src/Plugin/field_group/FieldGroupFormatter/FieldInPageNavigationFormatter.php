...
class FieldInPageNavigationFormatter extends FieldGroupFormatterBase {
  ...
  protected function hasVisibleLabel(string $type, string $field_name, array $processed_object): bool {
    if ($type === 'field' && in_array($processed_object[$field_name]['#label_display'], ['above', 'inline'])) {
      return TRUE;
    }
    if ($type === 'group') {
      if ($processed_object['#fieldgroups'][$field_name] === '' || isset($processed_object['#fieldgroups'][$field_name]->format_settings['show_label']) && $processed_object['#fieldgroups'][$field_name]->format_settings['show_label'] === FALSE) {
        return FALSE;
      }
      return TRUE;
    }
